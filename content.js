...
class YouTubeTranscriptExtractor {
  ...
  async handleCapturedTranscript(captureData) {
    try {
      ...
      // Early exit if not on a video page - prevents processing transcripts on homepage/search/etc
      const currentVideoId = this.getVideoId();
      if (!currentVideoId) {
        console.log(
          "🚫 Skipping transcript processing - not on a video page (no video ID found)"
        );
        console.log(
          "🚫 This is expected behavior on homepage, search pages, channel pages, etc."
        );
        return;
      }
        ...
        const sendTranscriptToBackground = async () => {
          try {
            ...
            if (!chrome.runtime?.id) {
              console.log(
                "🔄 Extension context not available - extension may have been reloaded"
              );
              console.log(
                "📝 Transcript captured but not sent to background (context invalidated)"
              );
              return;
            }
            ...
            chrome.runtime.sendMessage(transcriptData, (response) => {
              if (chrome.runtime.lastError) {
                if (
                  ...
                  console.log("🔄 Extension context invalidated during send");
                  console.log(
                    "📝 Transcript captured but not sent to background (extension was reloaded)"
                  );
                } else {
                  console.log(
